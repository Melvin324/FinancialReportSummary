...
/// <summary>
/// 本地公司库种子数据（~250 家 A 股 + 港股）。
/// 从 CompanySearchService 拆出来，纯数据、不含逻辑，方便单独维护和 review。
///
/// 维护须知：key 必须和 CompanyInfo.Code 保持一致（尤其是带前导零的港股代码，
/// 如 "00175" 不能写成 "0175"），且同一个 key 不能出现两次 —— C# 的
/// `["key"] = value` 索引器初始化器不会对重复 key 报错，只会静默用后面的覆盖前面的，
/// 曾经因此导致比亚迪(002594)的行业标签被意外改写，排查了很久才发现。
/// </summary>
public static class CompanyDirectory
{
    public static IReadOnlyDictionary<string, CompanyInfo> All => _companies;
    ...
    private static readonly Dictionary<string, CompanyInfo> _companies = new()
    {
        ...
        ["01109"] = new("01109", "华润置地", "huarun", "地产"),
